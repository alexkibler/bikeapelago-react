    ...
    private async Task LogErrorToDb(HttpContext context, Exception? ex)
    {
        try
        {
            ...
            if (request.ContentLength > 0)
            {
                ...
                // Redact passwords before truncating to avoid severing regex boundaries
                var redactedBody = Regex.Replace(fullBody, "\"(password|newPassword)\"\\s*:\\s*\"(?:[^\"\\\\]|\\\\.)*\"", "\"$1\": \"[REDACTED]\"", RegexOptions.IgnoreCase);
