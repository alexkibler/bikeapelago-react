    ...
    private async Task LogErrorToDb(HttpContext context, Exception? ex)
    {
        try
        {
            ...
            if (request.ContentLength > 0)
            {
                ...
                // Redact passwords before truncating to avoid severing JSON structure/regex boundaries
                var passwordRegex = new Regex("\"(password|newPassword)\"\\s*:\\s*\"(?:[^\"\\\\]|\\\\.)*\"", RegexOptions.IgnoreCase);
                fullBody = passwordRegex.Replace(fullBody, "\"$1\":\"[REDACTED]\"");
