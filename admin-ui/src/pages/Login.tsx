...
import { Lock, User as UserIcon, ShieldCheck, ChevronRight, Activity, Terminal } from 'lucide-react';
...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message ?? 'Gateway timeout or unauthorized link.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-[#09090b] border border-[#18181b] rounded-[48px] p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-12 animate-up">
            <div className="flex items-center gap-3 px-6 py-2 bg-indigo-500/5 border border-indigo-500/10 rounded-full mb-8 backdrop-blur-xl">
              ...
              <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-[0.2em]">Authentication Required</span>
            </div>
            ...
            <div className="flex items-center justify-center -space-x-3 mb-6">
              ...
              <div className="w-20 h-20 bg-indigo-600 rounded-[32px] flex items-center justify-center shadow-[0_0_40px_rgba(79,70,229,0.4)] relative z-10 -rotate-3 border-4 border-[#09090b]">
                <Activity size={40} strokeWidth={2.5} className="text-white drop-shadow-lg" />
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2 group">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.25em] ml-1 group-focus-within:text-indigo-400 transition-colors">Username or Email</label>
              <div className="relative">
                <UserIcon className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-indigo-500 transition-all" size={20} />
                <input
                  type="text"
            ...
            <div className="space-y-2 group">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.25em] ml-1 group-focus-within:text-indigo-400 transition-colors">Password</label>
              <div className="relative">
                <Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-indigo-500 transition-all" size={20} />
                <input
                  type="password"
                  placeholder="••••••••••••"
                  className="w-full bg-[#030304] border border-[#18181b] rounded-3xl pl-16 pr-6 py-5 text-zinc-100 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all font-medium text-lg placeholder:text-zinc-800"
            ...
            >
              ...
              {loading ? (
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <ChevronRight size={24} className="transform group-hover:translate-x-1.5 transition-transform duration-300" />
                </div>
              )}
            </button>
          </form>
          ...
          <footer className="mt-12 flex justify-center items-center px-2 opacity-30">
            <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Bikeapelago Admin Portal</div>
          </footer>
        </div>
      </div>
    </div>
