...
import { Search, Plus, Edit2, Trash2, MapIcon, Table as TableIcon, Layers, X, FileJson, ChevronDown, ChevronRight } from 'lucide-react';
...
export const DataExplorer: React.FC = () => {
  ...
  const fetchSchema = async () => {
    try {
      ...
      if (allTables.length > 0 && !selectedTable) setSelectedTable(allTables[0].table);
    } catch (err) {
  ...
  useEffect(() => { fetchSchema(); }, [token]);
  useEffect(() => { fetchData(); }, [selectedTable, token]);
  ...
  const handleSave = async (payload: any) => {
    try {
      ...
      const url = formMode === 'edit' && pk ? `/api/admin/data/${selectedTable}/${payload[pk]}` : `/api/admin/data/${selectedTable}`;
    ...
    <div className="flex flex-col h-full bg-[#09090b] border border-[#18181b] rounded-[40px] overflow-hidden shadow-2xl relative">
      ...
      <div className="p-8 border-b border-[#18181b] flex flex-wrap items-center justify-between gap-6 bg-zinc-950/20">
        <div className="flex items-center gap-4">
          ...
          <div>
             <h2 className="text-2xl font-black text-white tracking-tight font-outfit uppercase">Database Explorer</h2>
             <p className="text-xs text-zinc-600 font-bold tracking-widest uppercase">Browse and manage raw data collections</p>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-xl flex items-center gap-4">
          ...
          <button 
            onClick={() => { setSelectedRow(null); setFormMode('create'); }}
           ...
           <div className="p-6 space-y-4">
             {schemaGroups.map(group => (
               <div key={group.name} className="space-y-1">
                 ...
                 >
                   {expandedGroups.includes(group.name) ? <ChevronDown size={14} className="text-zinc-800 group-hover:text-zinc-400" /> : <ChevronRight size={14} className="text-zinc-800 group-hover:text-zinc-400" />}
                   {group.name}
                   ...
                   <div className="space-y-1 ml-1 pl-4 border-l border-zinc-900 py-1">
                     {group.tables.map(table => (
                       <button
                         ...
                         `}
                       >
                         <TableIcon size={14} className={selectedTable === table.table ? 'text-zinc-950' : 'text-zinc-700 group-hover:text-zinc-400'} />
                         <span className="flex-1 text-left text-xs tracking-tight truncate">{table.table}</span>
                         {selectedTable === table.table && <div className="absolute right-3 w-1.5 h-1.5 rounded-full bg-indigo-500" />}
                       </button>
                     ))}
                   </div>
                 )}
        ...
        <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar bg-[#020203]">
          {loading ? (
            <div className="flex items-center justify-center h-full gap-4">
               ...
               <span className="text-zinc-500 font-black tracking-widest uppercase text-sm">Loading Data</span>
            </div>
          ) : (
              ...
              <thead className="sticky top-0 bg-[#09090b] border-b border-[#18181b] z-20">
                <tr>
                  {currentTable?.columns.map(col => (
                    <th key={col.name} className="p-6 text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] whitespace-nowrap">
                       <div className="flex items-center gap-2">
                          {col.name}
                          {col.isPrimaryKey && <div className="p-1 px-1.5 bg-indigo-500/10 text-indigo-500 border border-indigo-500/20 rounded text-[8px]">PK</div>}
                       </div>
                    </th>
                  ))}
                  <th className="p-6 sticky right-0 bg-[#09090b] text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] text-center border-l border-[#18181b]">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#18181b]">
                {filteredData.map((row, idx) => (
                  <tr key={idx} className="group hover:bg-zinc-900/30 transition-all">
                    {currentTable?.columns.map(col => {
                      const val = row[col.name];
                      return (
                        <td key={col.name} className="p-6 text-[13px] text-zinc-400 font-medium tracking-tight max-w-xs truncate border-r border-[#18181b]/10">
                          {col.isSpatial ? (
                            <button
                            ...
                            <span className={col.isPrimaryKey ? 'text-white font-black font-mono' : ''}>
                               {String(val ?? '-')}
                            </span>
                          )}
                        </td>
                    ...
                    <td className="p-6 sticky right-0 bg-[#020203] group-hover:bg-[#09090b] transition-colors shadow-[-20px_0_30px_-10px_rgba(0,0,0,0.8)] border-l border-[#18181b]">
                       <div className="flex items-center gap-3 justify-center">
                         <button 
                            onClick={() => { setSelectedRow(row); setFormMode('edit'); }}
      ...
      {previewGeo && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-2xl" onClick={() => setPreviewGeo(null)} />
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-5xl rounded-[48px] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)] relative z-10 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500 border-indigo-500/10">
             <div className="p-10 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/40">
                <div className="flex items-center gap-6">
                   ...
                   <div>
                      <h3 className="font-black text-3xl text-white tracking-tighter font-outfit uppercase">Spatial Preview</h3>
                      <p className="text-zinc-500 font-bold tracking-[0.2em] text-[10px] uppercase">Geometric data visualization</p>
                   </div>
                </div>
                <button 
             ...
             <div className="p-10 bg-[#020203]">
                <SpatialPreview geoJson={previewGeo} height="500px" interactive={true} />
                <div className="mt-10 p-6 bg-zinc-950/80 rounded-[32px] border border-zinc-800/50 flex items-center gap-6">
                   <div className="p-3 bg-zinc-900 rounded-2xl text-zinc-600"><FileJson size={20} /></div>
