...
export const UserManagement: React.FC = () => {
  ...
  return (
    <div className='flex flex-col h-full space-y-8 animate-in fade-in duration-700'>
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-6'>
          ...
          <div>
            ...
            <p className='text-zinc-500 font-bold tracking-[0.2em] text-[10px] uppercase mt-1'>
              Manage system identities and access control
            </p>
          </div>
        </div>
      ...
      {message && (
        <div
          className={`p-4 rounded-2xl flex items-center gap-4 animate-in slide-in-from-top-2 duration-300 border ${
          ...
          <div className='bg-zinc-900 border border-zinc-800 w-full max-w-lg rounded-[48px] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,1)] relative z-10 animate-in zoom-in-95 duration-500'>
            ...
            <form onSubmit={handleCreateUser} className='p-10 space-y-6'>
              ...
              <div className='flex gap-4 mt-8'>
                <button
                  type='submit'
                  className='flex-1 h-14 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl font-black uppercase tracking-widest transition-all'
                >
                  Create Account
                </button>
                <button
                  type='button'
                  onClick={() => setShowCreateModal(false)}
                  className='px-8 h-14 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 rounded-2xl font-black uppercase tracking-widest transition-all'
                >
                  Cancel
                </button>
              </div>
          ...
          <table className='w-full text-left border-collapse'>
            <thead>
              <tr className='bg-zinc-950/50 border-b border-[#18181b]'>
                <th className='p-8 text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]'>
                  Profile
                </th>
                <th className='p-8 text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]'>
                  Email
                ...
                <th className='p-8 text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] text-right'>
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className='divide-y divide-[#18181b]'>
              {loading ? (
                ...
                <tr>
                  <td
                    colSpan={4}
                    className='p-20 text-center text-zinc-600 font-bold uppercase tracking-widest text-xs italic'
                  >
                  ...
                  >
                    ...
                    <td className='p-8'>
                      <div className='flex items-center gap-3 text-zinc-400 font-medium'>
                        ...
                        {user.email || (
                          <span className='text-zinc-800 italic text-xs'>
                            No email provided
                          </span>
                        )}
                      </div>
