...
import { Database, BarChart3, LogOut, Menu, X, Shield, Activity, Cpu, Users } from 'lucide-react';
      ...
      `}>
        <div className="flex flex-col h-full p-6">
          <div className="flex items-center gap-4 mb-12 px-2">
            ...
            <div>
               <h1 className="text-xl font-black text-white tracking-tighter font-outfit uppercase">Admin<span className="text-indigo-500">Panel</span></h1>
               <div className="flex items-center gap-1.5 opacity-50">
                  <div className="w-1 h-1 rounded-full bg-emerald-500" />
                  <span className="text-[10px] font-bold tracking-widest uppercase">Connected</span>
               </div>
            </div>
          </div>

          <nav className="flex-1 space-y-2">
            <div className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-4 ml-4">Management</div>
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
            ...
            <div className="bg-zinc-950/50 rounded-3xl p-4 border border-zinc-800/50 mb-6">
               <div className="flex items-center gap-3 mb-3">
                  <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center border border-zinc-700 overflow-hidden">
                     {user?.avatar ? <img src={user.avatar} className="w-full h-full object-cover" /> : <Shield size={16} className="text-zinc-500" />}
                  </div>
                  <div className="flex-1 overflow-hidden">
                     <p className="text-xs font-bold text-white truncate">{user?.name || 'Administrator'}</p>
                     <p className="text-[10px] text-zinc-600 font-mono tracking-tighter truncate uppercase">{user?.userName}</p>
                  </div>
               </div>
        ...
        <header className="h-20 border-b border-[#18181b] flex items-center justify-between px-8 bg-[#09090b]/50 backdrop-blur-md">
           <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-3 text-zinc-600">
                 <Cpu size={16} />
                 <span className="text-[10px] font-bold tracking-[0.3em] uppercase">Control Panel</span>
              </div>
           </div>
           ...
           <div className="flex items-center gap-4">
              <div className="px-4 py-1.5 bg-emerald-500/5 border border-emerald-500/10 rounded-full flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Database Linked</span>
              </div>
           </div>
        </header>
