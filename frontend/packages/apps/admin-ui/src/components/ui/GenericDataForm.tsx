...
export const GenericDataForm: React.FC<GenericDataFormProps> = ({
  table,
  columns,
  initialData,
  onSave,
    ...
    <div className='fixed inset-0 z-[60] flex items-center justify-center p-8 bg-zinc-950/80 backdrop-blur-xl animate-in fade-in duration-300'>
      <div className='bg-zinc-900 border border-zinc-800 w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl relative z-10 flex flex-col max-h-[90vh]'>
        <div className='p-6 border-b border-zinc-800 flex items-center justify-between'>
          <div>
            <h3 className='font-bold text-xl text-white'>
              {initialData ? 'Update' : 'Create'} {table} Record
            </h3>
            <p className='text-zinc-500 text-sm'>
              Fill in the fields defined in schema
            </p>
          </div>
          <button
            onClick={onCancel}
              ...
              <div key={col.name} className='space-y-2'>
                <label className='text-xs font-bold text-zinc-500 uppercase tracking-widest ml-1 flex justify-between'>
                  ...
                  <span className='text-[10px] text-zinc-700'>
                    {col.type} {col.isNullable ? '(Optional)' : '(Required)'}
                  </span>
                </label>
                ...
                {col.isSpatial ? (
                    ...
                    {formData[col.name] !== null &&
                      typeof formData[col.name] === 'object' && (
                        <div className='border border-zinc-800 rounded-2xl p-2 bg-zinc-950/20'>
                          <p className='text-[10px] font-bold text-zinc-700 uppercase tracking-widest mb-2 ml-2'>
                            Quick Preview
                          </p>
                          <SpatialPreview
                            geoJson={formData[col.name]}
                            height='140px'
                            interactive={false}
                          />
                        </div>
                  ...
                  <input
                    ...
                    value={renderValue(formData[col.name])}
                    onChange={(e) =>
                      handleChange(col.name, parseFloat(e.target.value))
                    }
        ...
        <div className='p-6 border-t border-zinc-800 flex items-center justify-end gap-3 bg-zinc-900/50'>
          <button
            onClick={onCancel}
            className='px-6 py-3 text-sm font-bold text-zinc-400 hover:text-white transition-all uppercase tracking-widest'
          >
            Discard
          </button>
          <button
