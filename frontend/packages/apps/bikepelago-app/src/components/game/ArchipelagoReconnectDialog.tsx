...
const ArchipelagoReconnectDialog = ({
  ...
  const inputClass =
    'w-full px-3 py-2 rounded-lg bg-[rgb(var(--color-surface-overlay))] border border-[var(--color-border-hex)] text-xs text-[var(--color-text-hex)] placeholder:text-[var(--color-text-muted-hex)] focus:outline-none focus:border-[var(--color-primary-hex)] transition-colors';
    ...
    <ModalShell onBackdropClick={onCancel} disableBackdropClick={isRetrying}>
      ...
      <div className='p-6' aria-labelledby='ap-reconnect-title'>
        ...
        <div className='flex items-start gap-3 mb-5'>
          <div className='flex-shrink-0 w-9 h-9 rounded-full bg-[var(--color-error-hex)]/15 flex items-center justify-center'>
            <svg
              ...
              className='text-[var(--color-error-hex)]'
            >
          ...
          <div className='flex-1 min-w-0'>
            <h2
              id='ap-reconnect-title'
              className='text-sm font-bold text-[var(--color-text-hex)] mb-0.5'
            >
              Archipelago Connection Failed
            </h2>
            <p className='text-[10px] text-[var(--color-text-muted-hex)] leading-relaxed break-words'>
              {error}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className='space-y-3'>
          <div>
            <label
              htmlFor='ap-reconnect-url'
              className='block text-[10px] font-bold text-[var(--color-text-muted-hex)] uppercase tracking-wider mb-1'
            >
              Server URL
            </label>
            <input
