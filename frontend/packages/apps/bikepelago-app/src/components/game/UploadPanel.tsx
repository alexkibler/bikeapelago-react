      ...
      {!analysisResult ? (
        <div className='flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[var(--color-border-hex)] rounded-2xl p-8 hover:border-[var(--color-primary-hex)]/50 transition-all group bg-[rgb(var(--color-surface-overlay))]'>
          ...
          {!selectedFile ? (
            <>
              ...
              <p className='text-sm font-bold text-[var(--color-text-muted-hex)]'>
                Choose a .FIT file
              </p>
              <p className='text-xs text-[var(--color-text-subtle-hex)] mt-2'>
        ...
        <div className='flex-1 flex flex-col'>
          ...
          <div className='flex-1 overflow-y-auto mb-6'>
            <h3 className='text-[10px] font-bold text-[var(--color-text-subtle-hex)] uppercase tracking-widest mb-4'>
              Locations Reached
            </h3>
            {analysisResult.newlyCheckedNodes.length > 0 ? (
              <div className='space-y-3'>
                {analysisResult.newlyCheckedNodes.map((node) => (
                  <div
                    key={node.id}
                    className='flex items-center gap-3 bg-[var(--color-success-hex)]/10 p-4 rounded-xl border border-[var(--color-success-hex)]/20 text-[var(--color-success-hex)]'
                  >
                    <CheckCircle2 className='w-5 h-5 shrink-0' />
                    <div className='flex flex-col'>
                      <span className='font-bold text-sm'>
                        Location{' '}
                        {node.apArrivalLocationId ??
                          node.ap_arrival_location_id}
                      </span>
          ...
          <div className='grid grid-cols-2 gap-4'>
            ...
            <button
              ...
              disabled={
                analysisResult.newlyCheckedNodes.length === 0 || loading
              }
