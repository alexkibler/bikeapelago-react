...
const ProgressionOverlay = ({ session }: { session: GameSession }) => {
  if (!session.center_lat || !session.center_lon || !session.radius)
    return null;
  ...
  const getWedgePoints = (
    ...
    for (let a = start; a <= end; a += step) {
      ...
      const lon =
        center[1] +
        (r / (111132 * Math.cos((center[0] * Math.PI) / 180))) * Math.cos(rad);
      points.push([lat, lon]);
    }
    ...
    points.push([
      ...
      center[1] +
        (r / (111132 * Math.cos((center[0] * Math.PI) / 180))) *
          Math.cos(finalRad),
    ]);
    ...
    if (innerR > 0) {
      ...
      for (let a = end; a >= start; a -= step) {
        ...
        const lon =
          center[1] +
          (innerR / (111132 * Math.cos((center[0] * Math.PI) / 180))) *
            Math.cos(rad);
        points.push([lat, lon]);
      }
  ...
  if (session.progression_mode === 'radius') {
    const steps = [
      ...
      { r: 0.5, unlocked: session.radius_step >= 1 },
      { r: 0.75, unlocked: session.radius_step >= 2 },
      { r: 1.0, unlocked: session.radius_step >= 3 },
    ];
  ...
  const quadrants = [
    {
      ...
      unlocked: session.north_pass_received,
    },
    { name: 'East', start: 45, end: 135, unlocked: session.east_pass_received },
    {
      ...
      unlocked: session.south_pass_received,
    },
    {
      name: 'West',
      start: 225,
      end: 315,
      unlocked: session.west_pass_received,
    },
  ];
  ...
  return (
    <>
      ...
      {quadrants.map((q) => (
        <div key={q.name}>
          <Polygon
            positions={getWedgePoints(
              q.start,
              q.end,
              outerRadius,
              q.unlocked ? 0 : hubRadius,
            )}
            pathOptions={{
    ...
    <div className='flex-1 relative'>
      ...
      {!activePanel && (
        <div className='absolute bottom-4 left-4 right-4 md:right-4 md:left-auto md:w-80 bg-[var(--color-surface-hex)]/90 backdrop-blur-md rounded-xl p-4 border border-[var(--color-border-strong-hex)] z-5 flex justify-between items-center shadow-2xl'>
          ...
          {routeData.gpxString && (
            <button
              onClick={() => downloadGPX(routeData.gpxString!)}
              className='px-3 py-2 bg-orange-600 hover:bg-orange-500 rounded-lg text-xs font-bold text-white transition-colors flex items-center gap-2'
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='14'
