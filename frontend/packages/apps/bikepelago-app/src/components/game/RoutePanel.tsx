...
const NodeListItem = ({ node, selected, onClick }: { node: MapNode; selected: boolean; onClick: () => void }) => (
  <button
    onClick={onClick}
...
const CategoryHeader = ({ title, count, color, isOpen, onClick }: {
  title: string; count: number; color: string; isOpen: boolean; onClick: () => void;
}) => (
  <button
    onClick={onClick}
    ...
    <div className="flex flex-col h-full bg-[var(--color-surface-hex)] text-[var(--color-text-hex)] overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-32">
        ...
        <div className="flex items-center gap-2 px-1">
          ...
          <span className="text-xs text-[var(--color-text-subtle-hex)]">{originStatus.label}</span>
          ...
          <div>
            ...
            {hasSelection && openCategories.Available && (
              <div className="flex items-center justify-between px-4 py-1.5 bg-violet-500/5 border-b border-violet-500/20">
                <span className="text-xs text-violet-400 font-medium">{selectedNodeIds.size} selected</span>
                <button onClick={clearSelectedNodes} className="text-xs text-violet-400 hover:text-violet-300 transition-colors">
                  Clear selection
                </button>
              </div>
            )}
          ...
          {openCategories.Checked && (
            <div className="max-h-64 overflow-y-auto bg-[var(--color-surface-hex)]/10">
              {checkedNodes.map(node => (
                <NodeListItem key={node.id} node={node} selected={false} onClick={() => {}} />
              ))}
