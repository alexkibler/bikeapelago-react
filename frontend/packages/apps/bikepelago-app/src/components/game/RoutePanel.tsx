...
const RoutePanel = ({ sessionId }: { sessionId: string }) => {
  ...
  // ⚡ Bolt: Single O(N) pass to categorize nodes instead of 3x .filter() calls O(3N).
  // Prevents main thread blocking during non-node state updates (e.g., toggles) on large node arrays.
