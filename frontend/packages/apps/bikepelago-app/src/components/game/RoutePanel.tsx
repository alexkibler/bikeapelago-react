...
const RoutePanel = ({ sessionId }: { sessionId: string }) => {
  ...
  // ⚡ Bolt Optimization: Group nodes into categories in a single O(N) pass memoized by nodes array.
  // This avoids three separate O(N) array filter passes and reduces redundant allocations on every render.
  const { availableNodes, checkedNodes, hiddenNodes } = useMemo(() => {
    const grouped = {
      availableNodes: [] as MapNode[],
      checkedNodes: [] as MapNode[],
      hiddenNodes: [] as MapNode[],
    };
    for (const node of nodes) {
      if (node.state === 'Available') grouped.availableNodes.push(node);
      else if (node.state === 'Checked') grouped.checkedNodes.push(node);
      else if (node.state === 'Hidden') grouped.hiddenNodes.push(node);
    }
    return grouped;
  }, [nodes]);
