...
const RoutePanel = ({ sessionId }: { sessionId: string }) => {
  ...
  // ⚡ Bolt Optimization: Group nodes in a single O(N) pass and memoize to avoid recalculation on unrelated re-renders
  // Impact: Reduces O(3N) filtering overhead and prevents main thread blocking during frequent UI state updates (e.g. toggling categories).
  const { availableNodes, checkedNodes, hiddenNodes } = useMemo(() => {
    const available: typeof nodes = [];
    const checked: typeof nodes = [];
    const hidden: typeof nodes = [];
