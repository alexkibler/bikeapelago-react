...
const RoutePanel = ({ sessionId }: { sessionId: string }) => {
  ...
  // Replaced multiple O(N) array .filter() calls with a single O(N) loop
  // memoized with useMemo. This reduces redundant array iterations and
  // allocations during unrelated component re-renders (like toggling UI state).
  // Expected Impact: Improves render speed when handling large node arrays.
