...
const InventoryPanel = () => {
  ...
  const signalAmpCount = activeCount(
    ITEMS.SIGNAL_AMPLIFIER.id,
    session?.signal_amplifiers_used,
  );
  ...
  const handleUseDetour = async () => {
    ...
    setIsUsing(true);
    try {
      await apiFetch(`${ENDPOINTS.ITEMS.DETOUR(sessionId)}?nodeId=${nodeId}`, {
        method: 'POST',
      });
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to use Detour';
      toast.error(message);
    } finally {
      setIsUsing(false);
    }
  ...
  const handleUseDrone = async () => {
    ...
    setIsUsing(true);
    try {
      await apiFetch(`${ENDPOINTS.ITEMS.DRONE(sessionId)}?nodeId=${nodeId}`, {
        method: 'POST',
      });
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to use Drone';
      toast.error(message);
    } finally {
      setIsUsing(false);
    }
  ...
  const handleUseSignalAmplifier = async () => {
    ...
    setIsUsing(true);
    try {
      await apiFetch(ENDPOINTS.ITEMS.SIGNAL_AMPLIFIER(sessionId), {
        method: 'POST',
      });
      ...
      const message =
        err instanceof Error
          ? err.message
          : 'Failed to activate Signal Amplifier';
      toast.error(message);
    } finally {
      setIsUsing(false);
    }
  ...
  const handleDebugUpdate = async (itemId: number, countStr: string) => {
    ...
    try {
      await apiFetch(
        `${ENDPOINTS.ITEMS.DEBUG_SET_ITEM_COUNT(sessionId)}?itemId=${itemId}&count=${count}`,
        { method: 'POST' },
      );
      triggerSync();
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : 'Debug update failed';
      toast.error(message);
    }
  };
  ...
  const ItemCount = ({ count, itemId }: { count: number; itemId: number }) => {
    ...
    return (
      <div className='text-xl font-black text-current opacity-80'>{count}</div>
    );
  };
      ...
      <div className='space-y-4'>
        ...
        <div className='bg-[rgb(var(--color-surface-overlay))] p-4 rounded-xl border border-[var(--color-border-strong-hex)] shadow-sm transition-all'>
          <div className='flex items-center justify-between mb-3 text-blue-500'>
            <div className='flex items-center gap-3'>
              ...
              <div>
                <h3 className='font-bold text-sm text-[var(--color-text-hex)]'>
                  The Detour
                </h3>
                <p className='text-[10px] text-[var(--color-text-muted-hex)] font-bold uppercase tracking-widest'>
                  Relocate Node
                </p>
              </div>
            </div>
          ...
          {detourCount > 0 && (
            ...
            >
              {selectedNodeIds.size === 1
                ? 'Use on Selected Node'
                : 'Select a node on map'}
            </button>
          )}
        </div>
        ...
        <div className='bg-[rgb(var(--color-surface-overlay))] p-4 rounded-xl border border-[var(--color-border-strong-hex)] shadow-sm transition-all'>
          <div className='flex items-center justify-between mb-3 text-orange-500'>
            ...
            <ItemCount
              count={signalAmpCount}
              itemId={ITEMS.SIGNAL_AMPLIFIER.id}
            />
          </div>
          {signalAmpCount > 0 && (
            <button
        ...
        {debugMode && (
          <div className='mt-8 space-y-2'>
            <h4 className='text-[10px] uppercase tracking-[0.2em] text-orange-500 mb-4 font-black'>
              Debug Progression
            </h4>
            {[
              ITEMS.PASS_NORTH,
              ...
              <div
                key={item.id}
                className='bg-[rgb(var(--color-surface-overlay))] p-3 rounded-lg border border-[var(--color-border-hex)] flex items-center justify-between'
              >
                <span className='text-xs font-bold text-[var(--color-text-muted-hex)]'>
                  {item.name}
                </span>
        ...
        {!debugMode && otherItems.length > 0 && (
          <div className='mt-8'>
            <h4 className='text-[10px] uppercase tracking-[0.2em] text-[var(--color-text-subtle-hex)] mb-4 font-bold'>
              Progression & Filler
            </h4>
              ...
              {otherItems.map((item) => (
                ...
                >
                  <span className='text-xs font-medium text-[var(--color-text-muted-hex)]'>
                    {item.name}{' '}
                    {item.count > 1 && (
                      <span className='opacity-60 ml-1'>(x{item.count})</span>
                    )}
                  </span>
