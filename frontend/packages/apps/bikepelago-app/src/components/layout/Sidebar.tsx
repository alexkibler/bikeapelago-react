...
import { type GamePanel, useGameStore } from '../../store/gameStore';
    ...
    <aside className='hidden md:flex flex-col w-64 h-screen bg-[var(--color-surface-hex)] border-r border-[var(--color-border-hex)] sticky top-0 shrink-0 z-[1001]'>
      ...
      <div className='p-6 flex items-center justify-between'>
        <Link to='/' className='flex items-center'>
          <span className='text-[var(--color-primary-hex)] uppercase font-extrabold text-2xl tracking-tighter italic'>
            bikeapelago
          </span>
        </Link>
      ...
      <nav className='flex-1 px-4 space-y-2 mt-4'>
        {isGamePage ? (
          <>
            <Link
              to='/'
              className='flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 text-[var(--color-text-muted-hex)] hover:text-[var(--color-text-hex)] hover:bg-[rgb(var(--color-surface-overlay))]'
            >
              <svg
          ...
          <>
            <Link
              to='/'
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${pathname === '/' ? 'bg-[var(--color-primary-hex)]/10 text-[var(--color-primary-hex)] shadow-inner shadow-primary/5' : 'text-[var(--color-text-muted-hex)] hover:text-[var(--color-text-hex)] hover:bg-[rgb(var(--color-surface-overlay))]'}`}
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='20'
            ...
            >
              ...
              >
                ...
                <path d='M4 19a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2H4Z' />
                <path d='M6 15v-2' />
                <path d='M10 15v-2' />
                <path d='M14 15v-2' />
                <path d='M18 15v-2' />
            ...
            <Link
              to='/yaml-creator'
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${pathname === '/yaml-creator' ? 'bg-[var(--color-primary-hex)]/10 text-[var(--color-primary-hex)] shadow-inner shadow-primary/5' : 'text-[var(--color-text-muted-hex)] hover:text-[var(--color-text-hex)] hover:bg-[rgb(var(--color-surface-overlay))]'}`}
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='20'
            ...
            <Link
              to='/settings'
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 ${pathname === '/settings' ? 'bg-[var(--color-primary-hex)]/10 text-[var(--color-primary-hex)] shadow-inner shadow-primary/5' : 'text-[var(--color-text-muted-hex)] hover:text-[var(--color-text-hex)] hover:bg-[rgb(var(--color-surface-overlay))]'}`}
            >
              <svg
                xmlns='http://www.w3.org/2000/svg'
                width='20'
      ...
      {user && (
        <div className='p-4 mt-auto border-t border-[var(--color-border-hex)]'>
          <button
            ...
            className='w-full flex items-center justify-center gap-3 p-3 rounded-xl hover:bg-[var(--color-error-hex)]/10 hover:text-[var(--color-error-hex)] text-[var(--color-text-muted-hex)] transition-all duration-200 group'
          >
            <svg
              xmlns='http://www.w3.org/2000/svg'
              width='18'
