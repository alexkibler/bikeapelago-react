    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-sm bg-[var(--color-surface-hex)] border border-[var(--color-border-hex)] rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-8">
          ...
          <h2 className="text-xl font-bold text-[var(--color-text-hex)] text-center mb-2">Delete Session?</h2>
          <p className="text-[var(--color-text-muted-hex)] text-center text-sm mb-8 leading-relaxed">
            Are you sure you want to delete <span className="text-[var(--color-text-hex)] font-semibold">"{session.name || session.ap_seed_name || 'Unnamed Session'}"</span>? 
      ...
      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              <div className="relative flex-grow flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  <div className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-tighter ${isAp(session) ? 'bg-orange-600 text-white' : 'bg-[var(--color-surface-alt-hex)] text-[var(--color-text-muted-hex)]'}`}>
          ...
          >
            ...
            <h3 className="text-xl text-[var(--color-text-hex)] font-bold mb-2">New Session</h3>
          ...
          >
            ...
            <h3 className="text-xl text-[var(--color-text-hex)] font-bold mb-2">APWorld Plugin</h3>
            <p className="text-[var(--color-text-muted-hex)] text-sm max-w-[200px]">
