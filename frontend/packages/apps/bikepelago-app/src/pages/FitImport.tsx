    ...
    return (
      <div className="min-h-screen p-6 max-w-lg mx-auto flex flex-col">
        <header className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-xl font-black text-[var(--color-text-hex)]">Ride Analysis</h1>
            <p className="text-xs text-[var(--color-text-subtle-hex)]">{selectedSession?.name ?? selectedSession?.ap_seed_name ?? 'Session'}</p>
          </div>
        </header>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-[var(--color-surface-alt-hex)] p-4 rounded-xl border border-[var(--color-border-hex)]">
            ...
            <div className="text-lg font-black mt-1 text-[var(--color-text-hex)]">
              ...
              <span className="text-xs font-normal text-[var(--color-text-subtle-hex)] ml-1">km</span>
            </div>
          </div>
          <div className="bg-[var(--color-surface-alt-hex)] p-4 rounded-xl border border-[var(--color-border-hex)]">
            ...
            <div className="text-lg font-black mt-1 text-[var(--color-text-hex)]">
              ...
              <span className="text-xs font-normal text-[var(--color-text-subtle-hex)] ml-1">min</span>
            </div>
          </div>
        </div>
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-sm">
                    Location {node.apArrivalLocationId ?? node.ap_arrival_location_id}
                  </span>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          >
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Confirm & Submit'}
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen p-6 max-w-lg mx-auto flex flex-col">
        <header className="flex items-center gap-3 mb-8">
          ...
          <h1 className="text-xl font-black text-[var(--color-text-hex)]">Import Ride</h1>
        </header>
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6">
          <div className="p-4 bg-[var(--color-surface-alt-hex)] rounded-xl border border-[var(--color-border-hex)] flex items-center gap-3 w-full">
            ...
            <span className="text-sm font-medium text-[var(--color-text-hex)] truncate">{file.name}</span>
          </div>

          <div className="p-4 bg-[var(--color-surface-alt-hex)] rounded-xl border border-[var(--color-border-hex)] w-full">
            <p className="text-[10px] font-bold text-[var(--color-text-subtle-hex)] uppercase tracking-widest mb-1">Session</p>
            <p className="font-bold text-[var(--color-text-hex)]">
              {selectedSession.name ?? selectedSession.ap_seed_name ?? 'Unnamed Session'}
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen p-6 max-w-lg mx-auto flex flex-col">
      ...
      <div className="flex items-center gap-2 mb-6 pl-1">
        ...
        <span className="text-xs text-[var(--color-text-muted-hex)] truncate">{file.name}</span>
      </div>
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2">
                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-tight ${session.ap_server_url ? 'bg-orange-600 text-white' : 'bg-[var(--color-surface-hex)] text-[var(--color-text-muted-hex)]'}`}>
