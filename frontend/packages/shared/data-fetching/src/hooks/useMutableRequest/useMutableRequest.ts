...
import type {
  ...
  MutationRequestCallbackArgs,
  MutationRequestFactoryCallback,
  MutationRequestFactoryCallbackArgs,
} from '../../types';
...
>(
  method: MethodType,
): MutationRequestCallback<TBody, TResponse, TSearchParams> {
  const { handleUnauthorized, token, baseUrl } = useDataFetchProviderCtx();

  const request: MutationRequestCallback<TBody, TResponse, TSearchParams> =
    useCallback(
      async (
        url: string,
        {
...
function buildMutableRequestFactory(method: MethodType) {
  ...
  >(
    path: TPath,
  ): MutationRequestFactoryCallback<TPath, TBody, TResponse, TSearchParams> {
