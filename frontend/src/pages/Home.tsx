...
const DeleteSessionDialog = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/60 backdrop-blur-md transition-opacity duration-300" onClick={!isDeleting ? onCancel : undefined} />
      
      {/* Panel */}
      <div className="relative w-full max-w-sm bg-[#0a0a0a] border border-neutral-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-8">
          ...
          <h2 className="text-xl font-bold text-white text-center mb-2">Delete Session?</h2>
          <p className="text-neutral-400 text-center text-sm mb-8 leading-relaxed">
            Are you sure you want to delete <span className="text-white font-semibold">"{session.name || session.ap_seed_name || 'Unnamed Session'}"</span>? 
            This action is permanent and will delete all nodes and routes associated with it.
          </p>
          ...
          <div className="flex gap-3">
            <button
              onClick={onCancel}
              disabled={isDeleting}
              className="flex-1 px-4 py-3 text-xs font-black uppercase tracking-widest rounded-xl border border-neutral-800 text-neutral-400 hover:bg-neutral-800 hover:text-white transition-all disabled:opacity-50"
            >
              Cancel
            </button>
            <button
  ...
  return (
    <div className="py-12 px-6 max-w-screen-xl mx-auto">
      ...
      <header className="mb-12 text-center max-w-2xl mx-auto">
        <h1 className="text-white text-4xl md:text-5xl font-black mb-4 tracking-tight text-glow">
            ...
            >
              ...
              <div className="relative flex-grow flex flex-col">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-neutral-500 group-hover:text-orange-500 transition-colors mr-8">
                    <PlayCircle className="w-6 h-6" />
                  </div>
                </div>

                <h3 className="text-xl text-white font-bold mb-3 group-hover:text-orange-500 transition-colors truncate pr-8">
