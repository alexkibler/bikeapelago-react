...
const ChatPanel = () => {
  ...
  const getMessageIcon = (type: ChatMessage['type']) => {
    switch (type) {
      case 'item': return <Zap className="w-3 h-3 text-[var(--color-primary-hex)]" />;
      case 'error': return <AlertCircle className="w-3 h-3 text-[var(--color-error-hex)]" />;
      case 'player': return <MessageSquare className="w-3 h-3 text-[var(--color-info-hex)]" />;
      default: return <Info className="w-3 h-3 text-[var(--color-text-subtle-hex)]" />;
    }
  };

  const getMessageColor = (type: ChatMessage['type']) => {
    switch (type) {
      case 'item': return 'text-[var(--color-primary-hex)]';
      case 'error': return 'text-[var(--color-error-hex)]';
      case 'player': return 'text-[var(--color-info-hex)]';
      case 'system': return 'text-[var(--color-text-muted-hex)]';
      default: return 'text-[var(--color-text-hex)]';
    }
  };
    ...
    <div className="flex flex-col h-full bg-[var(--color-surface-hex)] border-l border-[var(--color-border-hex)]">
      <div className="p-4 border-b border-[var(--color-border-hex)] flex items-center justify-between bg-[rgb(var(--color-surface-overlay))]">
        ...
        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[var(--color-surface-hex)]/40 border border-[var(--color-border-hex)]">
          ...
          <span className="text-[10px] font-bold text-[var(--color-text-subtle-hex)] uppercase tracking-tighter">{status}</span>
        </div>
      </div>
